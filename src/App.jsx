...
function App() {
  ...
  const handlePlayPause = () => { if (!playerRef.current) return; if (isPlaying) { playerRef.current.pauseVideo(); } else { playerRef.current.playVideo(); } setIsPlaying(!isPlaying); };
  const handleSeek = (newTime) => { if (!playerRef.current) return; const time = Number(newTime); playerRef.current.seekTo(time, true); setCurrentTime(time); };
  ...
  const onPlayerStateChange = (event) => { clearInterval(intervalRef.current); if (event.data === 1) { setIsPlaying(true); setDuration(playerRef.current.getDuration()); intervalRef.current = setInterval(() => { setCurrentTime(playerRef.current.getCurrentTime()); }, 250); } else { setIsPlaying(false); } };
  ...
  const handleSelectVideo = async (video) => { setSelectedVideo(video); setSearchResults([]); setIsLoading(true); setLyricsData(null); const youtubeTitle = video.snippet.title; const titleParts = youtubeTitle.split(' - '); let artist, songTitle; if (titleParts.length >= 2) { artist = titleParts[0].trim(); songTitle = titleParts.slice(1).join(' - ').trim(); } else { artist = video.snippet.channelTitle.replace(' - Topic', '').trim(); songTitle = youtubeTitle.trim(); } const cleanedSongTitle = songTitle.replace(/\[.*?\]/g, '').replace(/\(.*?\)/g, '').trim(); setLyricsData(await fetchLyrics(artist, cleanedSongTitle)); setIsLoading(false); };
  ...
  return (
    <div className="App">
      ...
      <main>
        {/* Only show search form if no song is selected */}
        {!selectedVideo && (
