...
function App() {
  ...
  const handleSelectVideo = async (video) => {
    ...
    if (titleParts.length >= 2) { artist = titleParts[0].trim(); songTitle = titleParts.slice(1).join(' - ').trim(); } else { artist = video.snippet.channelTitle.replace(' - Topic', '').trim(); songTitle = youtubeTitle.trim(); }
    ...
    try { const [lyricsResult, relatedResult] = await Promise.all([lyricsPromise, relatedPromise]); setLyricsData(lyricsResult); setRecommendations(relatedResult); } catch (error) { console.error("Failed to fetch data:", error); }
    setIsLoading(false);
  };
      ...
      {selectedVideo && (
        <footer className="playback-footer">
          <PlaybackControls
            ...
            thumbnailUrl={selectedVideo.snippet.thumbnails.default.url}
